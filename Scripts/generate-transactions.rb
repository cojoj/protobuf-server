...
CSV.open("../Resources/transactions_1.csv", "w", :col_sep => ";") do |csv|
    (0..1000).each do |i|
        csv << [i + 1000000, rand(0..1), Forgery('date').date, Forgery('date').date, Forgery('name').full_name, Forgery('name').full_name, Forgery('name').full_name, Forgery('bank_account').iban, Forgery('lorem_ipsum').title, Forgery('monetary').money]
    end
end

CSV.open("../Resources/transactions_2.csv", "w", :col_sep => ";") do |csv|
    (0..10000).each do |i|
        csv << [i + 2000000, rand(0..1), Forgery('date').date, Forgery('date').date, Forgery('name').full_name, Forgery('name').full_name, Forgery('name').full_name, Forgery('bank_account').iban, Forgery('lorem_ipsum').title, Forgery('monetary').money]
    end
end
